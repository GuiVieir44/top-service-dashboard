...
async function removeFolgaMassaNovembro() {
    ...
    try {
        ...
        // Filtrar apenas folgas do período a serem removidas
        const folgasParaRemover = absences.filter(a => 
            a.type === 'folga' && a.date >= startDate && a.date <= endDate
        );

        if (folgasParaRemover.length === 0) {
            showToast('ℹ️ Nenhuma folga encontrada no período para remover.', 'info');
            return { sucesso: true, folgasRemovidas: 0 };
        }

        console.log(`🗑️ Total de folgas a serem removidas: ${folgasParaRemover.length}`);

        for (const folga of folgasParaRemover) {
            // Assumindo que a função `removeAbsence` já existe e foi refatorada para Supabase
            await removeAbsence(folga.employeeId, folga.date);
        }
        
        console.log(`✅ ${folgasParaRemover.length} folgas removidas`);
        showToast(`✅ ${folgasParaRemover.length} folgas removidas!`, 'success');
        ...
        return {
            sucesso: true,
            folgasRemovidas: folgasParaRemover.length
        };
        
    } catch (error) {
        console.error('❌ Erro ao remover folgas:', error);
        showToast('Erro ao remover folgas em massa!', 'error');
        return {
            sucesso: false,
            erro: error.message
        };
