 ...
 */
async function corrigirPunchesProblematicos() {
    ...
    try {
        ...
        for (const [key, dayPunches] of Object.entries(punchesPorDiaFunc)) {
            ...
            console.log(`⚠️ ${key}: ${dayPunches.length} punches encontrados. Corrigindo...`);
            
            // Separar entradas e saídas e ordenar
            ...
            // Manter a primeira entrada e a última saída
            if (entradas.length > 0) {
                punchesAManter.push(entradas[0]);
                punchesARemover.push(...entradas.slice(1));
            }
            if (saidas.length > 0) {
                punchesAManter.push(saidas[saidas.length - 1]);
                punchesARemover.push(...saidas.slice(0, -1));
            }
        }

        if (punchesARemover.length === 0) {
            console.log('✅ Nenhum punch problemático para remover.');
            showToast('Nenhum punch para corrigir.', 'info');
            return;
        }

        console.log(`🗑️ Total de punches a serem removidos: ${punchesARemover.length}`);
        console.table(punchesARemover.map(p => ({ id: p.id, employeeId: p.employeeId, timestamp: p.timestamp, type: p.type })));

        // Usar a função `removePunch` refatorada para Supabase
        for (const punch of punchesARemover) {
            await removePunch(punch.id);
        }
        
        console.log(`\n✅ Correção concluída! ${punchesARemover.length} punches removidos.`);
        showToast(`✅ ${punchesARemover.length} punches corrigidos!`, 'success');
        ...
        console.error('❌ Erro ao corrigir punches:', e);
